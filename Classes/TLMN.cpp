...
USING_NS_CC;
using namespace ui;
Scene* TLMN::createScene()
{
    // 'scene' is an autorelease object
    auto scene = Scene::create();
    
    // 'layer' is an autorelease object
    auto layer = TLMN::create();
...
bool TLMN::init()
{
	...
	//tên game
	auto name = Sprite::create("Group-5.png");
	name->setPosition(Vec2(origin.x + backButton->getContentSize().width + callGm->getContentSize().width + name->getContentSize().width, origin.y + visibleSize.height - name->getContentSize().height / 2));
	this->addChild(name, 100);
	//tên người chơi
